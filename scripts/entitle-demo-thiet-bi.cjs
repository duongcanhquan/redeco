/**
 * Gán module `thiet-bi` vào hợp đồng active tenant demo.
 * node scripts/entitle-demo-thiet-bi.cjs
 */
const fs = require('fs');
...
async function main() {
  ...
  try {
    ...
    const mods = await pg.query(
      `select id, key from public.modules where key = 'thiet-bi' or key like 'thiet-bi.%'`,
    );
    if (mods.rows.length === 0) throw new Error('Chưa seed module thiet-bi');
    ...
    if (owner.rows[0]) {
      ...
      console.log('assigned owner manage on thiet-bi');
    }

    console.log('OK — mở /demo/equipment');
  } finally {
    await pg.end();
  }
}
