/**
 * Gán customiz (+ redeco-rfq) vào HĐ active tenant demo.
 * Chạy: node scripts/seed-modules.cjs && node scripts/entitle-demo-customiz-redeco-rfq.cjs
 */
const fs = require('fs');
...
async function main() {
  ...
  try {
    ...
    const mods = await pg.query(
      `select id, key from public.modules where key = 'customiz' or key like 'customiz.%'`,
    );
    if (mods.rows.length === 0) throw new Error('Chưa seed customiz — chạy seed-modules.cjs');
    ...
    console.log('OK — menu Kinh doanh hiện «Yêu cầu BG · REDECO»');
  } finally {
    await pg.end();
  }
}
