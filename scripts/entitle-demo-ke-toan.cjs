/**
 * Gán module `ke-toan` vào hợp đồng active của tenant demo.
 * Chạy: node scripts/entitle-demo-ke-toan.cjs
 */
const fs = require('fs');
...
async function main() {
  ...
  try {
    ...
    const mods = await pg.query(
      `select id, key from public.modules where key = 'ke-toan' or key like 'ke-toan.%'`,
    );
    if (mods.rows.length === 0) throw new Error('Chưa seed module ke-toan');
    ...
    if (owner.rows[0]) {
      ...
      console.log('assigned owner manage on ke-toan');
    }

    console.log('OK — /demo/accounting + Cài đặt → Kế toán (AR/COGS/AP flags)');
  } finally {
    await pg.end();
  }
}
