/**
 * Gán module `san-xuat` vào hợp đồng active của tenant demo.
 * Chạy: node scripts/entitle-demo-san-xuat.cjs
 */
const fs = require('fs');
...
async function main() {
  ...
  try {
    ...
    const mods = await pg.query(
      `select id, key from public.modules where key = 'san-xuat' or key like 'san-xuat.%'`,
    );
    if (mods.rows.length === 0) throw new Error('Chưa seed module san-xuat');
    ...
    if (owner.rows[0]) {
      ...
      console.log('assigned owner manage on san-xuat');
    }

    console.log('OK — /demo/production + Cài đặt → tab Sản xuất');
  } finally {
    await pg.end();
  }
}
