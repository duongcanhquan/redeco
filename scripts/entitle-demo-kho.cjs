/**
 * Gán module `kho` (+ subtree) vào hợp đồng active của tenant demo.
 * Chạy: node scripts/entitle-demo-kho.cjs
 */
const fs = require('fs');
...
async function main() {
  ...
  try {
    ...
    const mods = await pg.query(
      `select id, key from public.modules where key = 'kho' or key like 'kho.%'`,
    );
    if (mods.rows.length === 0) throw new Error('Chưa seed module kho — chạy seed-modules.cjs');
    ...
    // Gán owner demo đủ module kho (nếu dùng user_module_assignments)
    ...
    if (owner.rows[0]) {
      ...
      console.log('assigned owner manage on kho tree');
    }

    console.log('OK — đăng nhập demo và mở /demo/inventory');
  } finally {
    await pg.end();
  }
}
