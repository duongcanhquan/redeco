...
export function CalcPanel({
  ...
  if (requests.length === 0) {
    return (
      <div className="rounded-xl border border-panel/30 px-5 py-8 text-center space-y-2">
        <p className="font-semibold text-ink">Chưa có đề xuất</p>
        <p className="text-sm text-ink-muted">
          Import Excel hoặc thêm thủ công ở tab «Nhập đề xuất».
        </p>
      </div>
    );
  }
          ...
          >
            {requests.map((r) => (
              <option key={r.id} value={r.id}>
                {r.external_quote_no} — {r.end_customer || '—'}
              </option>
            ))}
          </select>
        </div>
      ...
      {selected && (
        <dl className="grid gap-2 text-sm sm:grid-cols-2">
          <div>
            ...
            <dd className="font-medium text-ink">{selected.product_name || '—'}</dd>
          </div>
          <div>
            <dt className="text-ink-muted">SL</dt>
            <dd className="font-medium text-ink">
              {selected.qty_expected || '—'} {selected.uom || ''}
            </dd>
          </div>
        </dl>
      )}
